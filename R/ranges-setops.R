...
#' @details Each of these functions acts on the rows of Ranges object. union
#' will return the entire range between two ranges assuming there are no gaps,
#' if there are gaps use \code{span} instead. intersect will update the Ranges object
#' with a hit column indicating whether or not the two ranges intersect.
#' setdiff will return the ranges for each row in x that are not in
