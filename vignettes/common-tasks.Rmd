...
1. Create a new GRanges object from CpG islands that stretches their ranges by 
their width while leaving their mid point fixed. 
...
2. How many distinct transcript identifers are there in exons?
...
that were merged together, and concatante the names of the exons that were 
merged.
...
Any of the above operations can be perfomed over subsets of the
...
chromHMM from ENCODE human embrionic stem cell ChIP-seq data. 
...
3. How many base pairs are represented in the genome by all predicted states?
...
3. Count the number of each enchancer element type that is exonic.
...
Then the results can be put togeher using `rbind()` or plotted using
your favourite plotting pacakge. 
...
(These are adapted from the HelloRanges tutorial.) 
We will work through the following exercises together 

First, let's load the data required, which is are enhancer elements  
