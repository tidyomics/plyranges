...
given query or subject. We can achieve this with an intersect overlap join 
and mutate. Since an overlap inner join will keep the the original query and
subject ranges as metadata columns we can combine it with filter. 
In this example we filter intersecting ranges where the proportion of overlap 
on cpg islands is less than half.

```{r}
olap <- join_overlap_intersect(cpg, exons) %>%
  filter(width / width(granges.x) >= 0.5)
olap
```
