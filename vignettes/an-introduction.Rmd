...
combination with either `set_width` or `stretch`.
...
ggbio::tracks(query = ggbio::autoplot(query),
              subject = ggbio::autoplot(subject))
```

The first function, `join_overlap_intersect` will return a `Ranges`` object 
where the start, end, and width coordinates correspond to the amount of any overlap between the left and right input `Ranges`. It also returns any metadata
in the subject range if the subject overlaps the query. Note that by default
the join functions ignore strandedness.
...
ggbio::tracks(query = ggbio::autoplot(query),
              subject = ggbio::autoplot(subject),
              `inner join` = ggbio::autoplot(inner_rng))
```
...
ggbio::tracks(query = ggbio::autoplot(query),
              subject = ggbio::autoplot(subject),
              `left outer join` = ggbio::autoplot(left_rng))
```
...
We provide convienence functions via `rtracklayer` and `GenomicAlignments` 
