...
test_that("summarise evaluates correctly", {
  df <- data.frame(mean = 4.8)
  expect_equal(summarise(ir1, mean = mean(score)), df)
  expect_equal(summarise(gr1, mean = mean(score)), df)
  gdf <- data.frame(grp = c("A", "B"), score = c(3.6, 6),
