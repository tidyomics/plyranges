...
- paired:
  ...
  Whether to treat alignments as paired end (TRUE) or single end
...
- `has_unmapped_mate` Select reads with mapped (FALSE) or unmapped
  (TRUE) mates.

- `is_minus_strand` Select reads aligned to plus (FALSE) or minus (TRUE)
  strand.

- `is_mate_minus_strand` Select reads where mate is aligned to plus
  (FALSE) or minus (TRUE) strand.

- `is_first_mate_read` Select reads if they are the first mate (TRUE) or
  not (FALSE).

- `is_second_mate_read` Select reads if they are the second mate (TRUE)
  or not (FALSE).
...
- `is_duplicate` Select reads that are unduplicated (FALSE) or
  duplicated (TRUE). This may represent reads that are PCR or optical
  duplicates.

## See also

`Rsamtools::`[`BamFile()`](https://rdrr.io/pkg/Rsamtools/man/BamFile-class.html),[`GenomicAlignments::readGAlignments()`](https://rdrr.io/pkg/GenomicAlignments/man/readGAlignments.html)
